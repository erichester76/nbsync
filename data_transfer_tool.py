...
class DataTransferTool:
    ...
    def resolve_nested_context(self, item):
        ...
        def get_nested_value(obj, attr_path):
            ...
            try:
                for attr in attrs:
                    timer.start_timer(f"Resolve Nested Context {attr_path} {attrs} {attr}")
                    ...
                    if current_obj is None:
                        break
                    timer.stop_timer(f"Resolve Nested Context {attr_path} {attrs} {attr}")
                return current_obj
                    ...
                    for item in source_data:
                        ...
                        for dest_field, field_info in mappings.items():
                            if 'source' in field_info:
                                ...
                                rendered_source_value = template.render(item)
